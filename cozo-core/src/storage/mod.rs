...
pub trait StoreTx<'s>: Sync {
    ...
    /// The default implementation calls [`range_scan`](Self::range_scan) and converts
    /// via [`decode_tuple_from_kv`](crate::decode_tuple_from_kv).
    ...
    /// Scan on a range with a certain validity, skipping tuples that don't apply
    /// at the given validity point. See impl docs for the full semantics.
